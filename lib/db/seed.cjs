...
async function main() {
  ...
  if (Number(katCount.rows[0].c) === 0) {
    // 5 kategori with gender split for Anak (k_anak_l / k_anak_p) and
    // single-female Dewasa (Ibu-Ibu). Color palette per-section so warga
    // can tell at a glance which age/gender group they're looking at.
    const kats = [
      // [id, nama, icon, min, max, urutan, auto_age, color_bg, color_text, color_border]
      ["k_balita",   "Balita",              "fa-baby",         0,   4,   0, 1, "#FCE7F3", "#9D174D", "#FBCFE8"],
      ["k_anak_l",   "Anak (Laki-laki)",    "fa-child",        5,  11,   1, 1, "#DBEAFE", "#1E40AF", "#BFDBFE"],
      ["k_anak_p",   "Anak (Perempuan)",    "fa-child-dress",  5,  11,   2, 1, "#FCE7F3", "#9D174D", "#FBCFE8"],
      ["k_remaja",   "Remaja",              "fa-user",        12,  17,   3, 0, "#EDE9FE", "#5B21B6", "#DDD6FE"],
      ["k_dewasa_p", "Ibu-Ibu",             "fa-user-tie",    18, 999,   4, 1, "#FEF3C7", "#92400E", "#FDE68A"],
    ];
    for (const k of kats) {
      await client.execute({
        sql: "INSERT INTO kategori (id, nama, icon, min, max, urutan, auto_age, color_bg, color_text, color_border) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        args: k,
      });
    }
    console.log("✓ 5 kategori seeded (Balita, Anak L/P, Remaja, Ibu-Ibu) — gender-split for Anak, female-only Dewasa");
  } else {
    // Idempotent: ensure k_dewasa_p has auto_age=1 (single-female Dewasa)
    await client.execute("UPDATE kategori SET auto_age = 1 WHERE id = 'k_dewasa_p'");
  ...
  if (Number(lombaCount.rows[0].c) === 0) {
    ...
    // Gender split: "k_anak" is now k_anak_l + k_anak_p, "k_dewasa" is now k_dewasa_p
    ...
    // Per-kategori PJ pairs. PJ for k_anak is duplicated for k_anak_l + k_anak_p
    // (same person oversees both gender groups within the anak age range).
    const seeds = [
      {
        ...
        pjList: [
          { kat: "k_anak_l",  nama: "Bu Yuni (Pos Pelayanan Anak)",   kontak: "0812-1111-0001" },
          { kat: "k_anak_p",  nama: "Bu Yuni (Pos Pelayanan Anak)",   kontak: "0812-1111-0001" },
          { kat: "k_remaja",  nama: "Mas Rio (Karang Taruna)",         kontak: "0812-1111-0002" },
          { kat: "k_dewasa_p", nama: "Pak H. Bowo (RT 02)",            kontak: "0812-1111-0003" },
        ],
      },
      {
        ...
        pjList: [
          ...
          { kat: "k_dewasa_p", nama: "Pak Udin (RT 04)",    kontak: "0813-2222-0002" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_dewasa_p", nama: "Pak H. Sudirman", kontak: "0814-3333-4444" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_dewasa_p", nama: "Pak Lurah", kontak: "0815-5555-6666" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_anak_l", nama: "Bu Aminah", kontak: "0816-7777-8888" },
          { kat: "k_anak_p", nama: "Bu Aminah", kontak: "0816-7777-8888" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_anak_l",  nama: "Kak Nisa (Guru TK)",   kontak: "0817-9999-0001" },
          { kat: "k_anak_p",  nama: "Kak Nisa (Guru TK)",   kontak: "0817-9999-0001" },
          { kat: "k_remaja",  nama: "Pak RW",                kontak: "0817-9999-0002" },
          { kat: "k_dewasa_p", nama: "Ibu PKK",             kontak: "0817-9999-0003" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_anak_l", nama: "Ibu PKK", kontak: "0818-1111-3333" },
          { kat: "k_anak_p", nama: "Ibu PKK", kontak: "0818-1111-3333" },
        ],
      },
      {
        ...
        pjList: [
          { kat: "k_anak_l", nama: "Bunda RT", kontak: "0819-2222-4444" },
          { kat: "k_anak_p", nama: "Bunda RT", kontak: "0819-2222-4444" },
        ],
      },
    ];
    ...
    console.log(`✓ ${seeds.length} lomba seeded with per-kategori PJ (gender-split for k_anak, k_dewasa renamed to k_dewasa_p)`);
  } else {
